...
fun aboutapp(navHostController: NavHostController) {
                ...
                ListItem({
                    ...
                    modifier = Modifier.clickable {
                        if (autoupdate?.isNotEmpty() == true) {
                            ...
                            Toast.makeText(
                                context,
                                "Unable to get data try after sometime",
                                Toast.LENGTH_SHORT
                            ).show()
                        }
    ...
    if(showwarning){
        AlertDialog(onDismissRequest = { showwarning=false},
            ...
            confirmButton = { TextButton(onClick = { viewModel.cleardb();viewModel.clearwebsites();showwarning=false}) {
                Text("Clear")

            }},
            title = {Text("Clear local data",modifier=Modifier.fillMaxWidth(), textAlign = TextAlign.Center)},
